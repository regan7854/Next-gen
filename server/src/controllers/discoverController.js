import { getDb, dbAll, dbGet } from '../lib/database.js';
...
export async function searchInfluencers(req, res, next) {
  try {
    ...
    if (location) { sql += ` AND (u.location LIKE ? OR ip.audience_location LIKE ?)`; params.push(`%${location}%`, `%${location}%`); }
    if (q) { sql += ` AND (u.display_name LIKE ? OR u.biography LIKE ? OR ip.niche LIKE ?)`; params.push(`%${q}%`, `%${q}%`, `%${q}%`); }
    ...
    const results = rows.map((r) => ({
      ...
      audienceLocation: r.audience_location,
      minRate: r.min_rate,
      maxRate: r.max_rate,
...
export async function searchBrands(req, res, next) {
  try {
    ...
    if (q) { sql += ` AND (u.display_name LIKE ? OR bp.company_name LIKE ? OR bp.product_type LIKE ?)`; params.push(`%${q}%`, `%${q}%`, `%${q}%`); }
    ...
    const results = rows.map((r) => ({
      ...
      campaignGoals: r.campaign_goals,
...
export async function getRecommendations(req, res, next) {
  try {
    ...
    const user = await dbGet(db, 'SELECT role FROM users WHERE id = ?', [userId]);
    if (!user) return res.status(404).json({ message: 'User not found' });

    let results = [];

    if (user.role === 'brand') {
      // brand gets influencer recommendations
      const brand = await dbGet(db, 'SELECT * FROM brand_profiles WHERE user_id = ?', [userId]);
      if (!brand) return res.json({ results: [] });

      const influencers = await dbAll(db, `
      ...
      results = influencers.map((inf) => {
        ...
        return {
          ...
          minRate: inf.min_rate,
          maxRate: inf.max_rate,
          matchScore: score,
        };
      }).filter((r) => r.matchScore > 0).sort((a, b) => b.matchScore - a.matchScore).slice(0, 20);

    } else if (user.role === 'influencer') {
      // influencer gets brand recommendations
      const inf = await dbGet(db, 'SELECT * FROM influencer_profiles WHERE user_id = ?', [userId]);
      if (!inf) return res.json({ results: [] });

      const brands = await dbAll(db, `
      ...
      results = brands.map((brand) => {
        ...
        return {
          ...
          avatarColor: brand.avatar_color,
          location: brand.location,
          industry: brand.industry,
          campaignGoals: brand.campaign_goals,
...
export async function getTrending(_req, res, next) {
  try {
    ...
    // Top 10 Influencers — scored by followers + engagement + collab count + reviews
    const topInfluencers = await dbAll(db, `
      SELECT
        ...
        (
          COALESCE(ip.instagram_followers, 0) +
          COALESCE(ip.tiktok_followers, 0) +
          COALESCE(ip.youtube_subscribers, 0)
        ) as total_followers,
        (
          COALESCE(ip.instagram_followers, 0) +
          COALESCE(ip.tiktok_followers, 0) +
    ...
    // Top 10 Brands — scored by budget + collab count + reviews
    const topBrands = await dbAll(db, `
      SELECT
        u.id, u.display_name, u.biography, u.avatar_color, u.location,
        bp.company_name, bp.industry, bp.website, bp.product_type,
        bp.target_audience, bp.campaign_goals,
        bp.min_budget, bp.max_budget,
        bp.preferred_platforms, bp.preferred_categories,
        COALESCE((SELECT AVG(rating) FROM reviews WHERE reviewee_id = u.id), 0) as avg_rating,
        COALESCE((SELECT COUNT(*) FROM reviews WHERE reviewee_id = u.id), 0) as review_count,
        COALESCE((SELECT COUNT(*) FROM collaboration_requests WHERE (sender_id = u.id OR receiver_id = u.id) AND status = 'accepted'), 0) as collab_count,
        (
...
function calculateMatchScore(brand, influencer, perspective) {
  ...
  const prefCats = (brand.preferred_categories || '').toLowerCase();
  const infCat = (influencer.category || '').toLowerCase();
  if (prefCats && infCat && prefCats.includes(infCat)) {
    score += 30;
  } else if (infCat) {
    score += 5; // partial
  }
  ...
  // Platform match (20 pts)
  const prefPlat = (brand.preferred_platforms || '').toLowerCase();
  let platScore = 0;
  if (prefPlat) {
    if (prefPlat.includes('instagram') && influencer.instagram_followers > 0) platScore += 7;
    if (prefPlat.includes('tiktok') && influencer.tiktok_followers > 0) platScore += 7;
    if (prefPlat.includes('youtube') && influencer.youtube_subscribers > 0) platScore += 6;
  } else {
    // no preference — any platform with followers counts
    if (influencer.instagram_followers > 0) platScore += 7;
    if (influencer.tiktok_followers > 0) platScore += 7;
    if (influencer.youtube_subscribers > 0) platScore += 6;
  }
  score += platScore;
  factors += 20;
  ...
  const budgetOverlap = checkBudgetOverlap(
    brand.min_budget, brand.max_budget,
    influencer.min_rate, influencer.max_rate
  );
  score += Math.round(budgetOverlap * 25);
  factors += 25;

  // Audience location match (15 pts)
  const targetAud = (brand.target_audience || '').toLowerCase();
  const audLoc = (influencer.audience_location || '').toLowerCase();
  if (targetAud && audLoc) {
  ...
  // Engagement bonus (10 pts)
  const engagement = influencer.instagram_engagement || 0;
